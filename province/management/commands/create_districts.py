...
from province.models import City, District, Center, Country

class Command(BaseCommand):
    help = 'İlçeleri geocode ile oluşturur'

    def handle(self, *args, **options):
        # Google Maps API client
        gmaps = googlemaps.Client(key='AIzaSyBEHrVmu7zD2SYlo3tKLlXJVxfNeZLIhDI')
        ...
        for district_data in DISTRICTS:
            try:
                # Şehri bul
                city = City.objects.get(id=district_data['il_id'])
                
                # Geocode için sorgu oluştur (Şehir adı + İlçe adı)
                search_query = f"{city.name} {district_data['name']} Turkey"
                
                # Google Maps Geocoding API ile konum bul
                geocode_result = gmaps.geocode(search_query)
                
                if geocode_result:
                    location = geocode_result[0]['geometry']['location']
                    
                    # Center objesi oluştur
                    center = Center.objects.create(
                        latitude=str(location['lat']),
                        longitude=str(location['lng'])
                    )
                    
                    # District oluştur veya güncelle
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"{district.name} - {city.name} {action} "
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"{district_data['name']} için geocode sonucu bulunamadı"
                        )
                    )
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"Şehir bulunamadı (id: {district_data['il_id']})"
                    )
                )
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(
                        f"{district_data['name']} işlenirken hata: {str(e)}"
                    )
                )